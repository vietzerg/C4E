...
def csv_reader(config_file):
    ...
    with open(config_file, "r") as csvfile:
            ...
            for row in csvreader:
                if row["type"] == "height":
                    csv_read["height"] = int(row["value"])
                elif row["type"] == "width":
                    csv_read["width"] = int(row["value"])
                elif row["type"] == "obstacle":
                    a_obstacle = SKObject(int(row["x"]), int(row["y"]), " O ")
                    csv_read["obstacles"].append(a_obstacle)
                    csv_read["objects"].append(a_obstacle)
                elif row["type"] == "box":
                    a_box = SKObject(int(row["x"]), int(row["y"]), " B ")
                    csv_read["boxes"].append(a_box)
                    csv_read["objects"].append(a_box)
                elif row["type"] == "storage":
                    a_storage = SKObject(int(row["x"]), int(row["y"]), " S ")
                    csv_read["storages"].append(a_storage)
                    csv_read["objects"].append(a_storage)
                elif row["type"] == "character":
                    csv_read["chaien"] = SKObject(int(row["x"]), int(row["y"]), " C ")
                    csv_read["objects"].append(csv_read["chaien"])
                elif row["type"] == "blink":
                    a_blink = SKObject(int(row["x"]), int(row["y"]), " - ")
                    csv_read["blinks"].append(a_blink)
                    csv_read["objects"].append(a_blink)

    return csv_read

#print (csv_reader("game_cfg.csv")) 
...
class Map:
    def __init__(self, csv_read):
        
        # SETTING UP THE PROPERTIES OF THE MAP
        self.mana = 0
        self.obstacles = csv_read["obstacles"]
        self.boxes = csv_read["boxes"]
        self.storages = csv_read["storages"]
        self.blinks = csv_read["blinks"]
        self.objects = csv_read["objects"]
        self.chaien = csv_read["chaien"]
        self.height = csv_read["height"]
        self.width = csv_read["width"]
...
# map1 = Map(csv_reader("game_cfg.csv"))
# while True:
#     map1.printer()
